...
public class FarmSim
{
    ...
    public int Harvest(Cell c) {
    if (!world.plots.TryGetValue(c, out var p) || p.IsEmpty) return 0;

    var def = crops[p.cropId];
    bool mature = p.stage >= def.growthFrames.Length - 1;
    if (!mature) return 0;

    int yield = def.harvestYield;

    if (def.regrowAfterHarvest) {
        // drop back one stage so it can regrow
        p.stage = Mathf.Max(0, def.growthFrames.Length - 2);
        p.stageTime = 0f;
    } else {
        // clear the plot
        p.cropId = null;
